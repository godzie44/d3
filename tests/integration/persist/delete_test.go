...
func (d *DeleteTS) TestDeleteEntity() {
	...
	profile, err := rep.FindOne(d.ctx, rep.Select().AndWhere("profile_p.id = 1001"))
	d.NoError(err)
...
func (d *DeleteTS) TestDeleteWithRelations() {
	...
	shop, err := rep.FindOne(d.ctx, rep.Select().AndWhere("shop_p.id = 1001"))
	d.NoError(err)
...
func (d *DeleteTS) TestDeleteWithManyToMany() {
	...
	book, err := rep.FindOne(d.ctx, rep.Select().AndWhere("book_p.id = 1001"))
	d.NoError(err)
