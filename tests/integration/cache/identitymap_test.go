...
func (o *IMCacheTS) TestNoQueryCreateForCachedEntities() {
	...
	_, err = repository.FindOne(ctx, repository.CreateQuery().AndWhere("im_test_entity_1.id = ?", 1))
	o.Assert().NoError(err)
	...
	_, err = repository2.FindOne(ctx, repository2.CreateQuery().AndWhere("im_test_entity_2.id = ?", 1))
	o.Assert().NoError(err)
