...
func (o *IMCacheTS) TestNoQueryCreateForCachedEntities() {
	...
	_, err := repository.FindOne(repository.CreateQuery().AndWhere("test_entity_1.id = ?", 1))
	o.Assert().NoError(err)
	...
	_, err = repository2.FindOne(repository.CreateQuery().AndWhere("test_entity_2.id = ?", 1))
